...
### 1-6. 실습: 평균보다 많이 쓴 달은 언제인가? (17분)

1-3에서 강사 화면으로 본 스칼라 서브쿼리를 이번에는 **직접** 만들어 봅니다.
한 번에 완성하려 하지 말고 STEP을 순서대로 밟으세요. **앞 STEP의 결과가 다음 STEP의 재료**가 됩니다.

아래를 그대로 복사해 콘솔에 붙여 넣고, 각 주석 아래에 쿼리를 채워 나가면 됩니다.

```sql
/*Week8 Day1*/
--실습1. "평균보다 많이 쓴 달은 언제인가?"
--STEP1) 월별 소비금액은?
--쿼리:
...
--STEP3) 평균보다 많이 쓴 달은 총 몇개 이며, 그 중 최솟값와 최댓값은?
--정답: 평균보다 많이 쓴 달의 개수는 170개이며, 최댓값은 7230712.13, 최솟값은 3106839.5 입니다.
--쿼리:
```

STEP별 힌트입니다. 막히면 순서대로 펼쳐 보세요.

- (1) **STEP 1** — 그레인은 "한 행 = 월"입니다. 날짜를 월로 자르고(`DATE_TRUNC`) 금액을 합칩니다. 여기서 나온 결과가 몇 행인지 세어 두세요.
...
복선 하나. 7주차 Day 2 마지막 문제에서 채널별 사기율을 구했던 것 기억나시죠. **이 데이터의 전체 사기율은 약 0.12%**입니다 — 이 작은 숫자를 기억해 두세요. 내일 3교시 사기 탐지(FDS)에서 "정확도 99.9% 모델의 함정"과 함께 돌아옵니다.
...
### 3-4. 실습 4: "Champions는 누구이고, 어디에 쓰는가" (23분)
...
### 3-5. 마무리: 코호트 브리지와 내일 예고 (3분)

오늘 만든 RFM에서 한 걸음만 더 가면 **코호트 잔존율**이 나옵니다. RFM의 R이 "마지막 거래가 언제였나"라는 스냅숏 한 장이라면, 코호트 잔존율은 "첫 거래 후 매달 살아있었나"라는 **시간축 필름**입니다. 금융권 고객 유지율 분석의 기본형이죠.

> 💡 **더 해보고 싶다면 (선택 도전).** 첫 거래월별로 고객을 묶어 월별 유지율을 구해 보세요. 오늘 배운 것만으로 풀립니다.
> (1) 사용자별 첫 거래월 — `MIN(ym) OVER (PARTITION BY user_id)`
> (2) 경과월 — `DATE_DIFF(거래월, 첫 거래월, MONTH)`
> (3) 코호트 × 경과월 집계 — `COUNT(DISTINCT user_id)`를 코호트 크기로 나눔
> 검증: 경과월 0의 유지율은 정의상 1.00입니다. 아니라면 분모를 잘못 잡은 것입니다.
...
| 증상 | 원인 | 해결 |
| --- | --- | --- |
| STEP 3 결과가 정답과 다름 | STEP 1의 그레인 또는 금액 정제 방식 차이 | 월별 합계(STEP 1)부터 대조 |
| STEP 2 평균이 수백 단위로 나옴 | 거래 단위 평균을 구함 (월별 합계의 평균이 아님) | STEP 1 결과 위에서 `AVG` |
| `Aggregate function not allowed in WHERE` | 집계를 `WHERE`에 직접 씀 | 스칼라 서브쿼리로 감싸기 |
| anti-join 결과가 0행 | 정상일 수 있음 (합성 데이터라 모든 카드에 거래가 있을 수 있음) | 기간을 좁혀 다시 확인 |
