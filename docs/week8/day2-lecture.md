...
### 1-1. 도입: 금융 시계열 분석의 지도 (4분)

퀀트·금융공학 커뮤니티의 유명한 자료 모음(awesome-quant)을 열어 보면 분야가 이렇게 나뉩니다 — 기술적 지표, 시계열 분석, 리스크 분석, 팩터 분석…. 전부 파이썬 라이브러리 목록처럼 보이지만, **그 계산의 뼈대는 대부분 어제 배운 윈도우 함수입니다.**

- (1) 이동평균(SMA) → `AVG() OVER (… ROWS …)`
- (2) 변동성(rolling volatility) → `STDDEV_SAMP() OVER (… ROWS …)`
- (3) 이상 탐지(z-score) → 위 둘의 조합
- (4) 볼린저 밴드 → 이동평균 ± 2 × 변동성

오늘 1교시의 목표: 카드 소비 시계열에서 **"이상하게 튄 날"**을 SQL로 찾아내는 것.

### 1-2. 시계열의 전제: 균일한 간격 만들기 (7분)
...
- 힌트: 어제 실습 2-②의 재사용 — 오늘은 연령대 없이 전체 기준.
...
보너스 아이디어: `cards.card_on_dark_web = 'true'`인 카드의 거래에 가중 플래그 — 과제 자유 룰의 힌트입니다.
...
- 토론: 내 룰이 놓친 사기(FN)를 3건만 눈으로 보세요. 공통점이 보입니까? — 그 관찰이 과제 Part B(자유 룰 설계)의 씨앗입니다.

### 3-7. 마무리: 과제와 내일 예고 (3분)

- (1) 과제 Part B: **자기만의 FDS 룰 1개 설계** — 가설을 먼저 쓰고(없으면 절반 인정), 구현하고, 성적표를 뽑고, 수업 룰과 비교 한 단락. 단, `is_fraud` 같은 정답 라벨을 룰 조건에 쓰면 구현 0점입니다 — 시험 문제를 답안지 보고 푸는 것이니까요.
- (2) 내일 예고: 사흘째, 시선을 바꿉니다. **"분석할 데이터가 늘 남이 만들어 준 것뿐일까?"** — 내일은 대한민국 전자공시(DART)에서 재무제표를 직접 수집해, 여러분의 BigQuery에 쌓고, 오늘까지의 SQL로 기업을 분석합니다.
